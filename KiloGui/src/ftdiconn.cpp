...
void FTDIConnection::sendProgram(QString file) {
   // QMessageBox::information(nullptr, "Debug", QString("sendProgram: %1").arg(file));
    if (ftdic != NULL) {
        //QMessageBox::information(nullptr, "Debug", "FTDI connected");
        ...
        //QMessageBox::information(nullptr, "Debug", "FTDi pointer is NULL");
...
void FTDIConnection::programLoop() {
    if (ftdic != NULL && delay.elapsed() > 130) {
        if (page >= page_total) {
            qDebug() << "sending boot start packet";
            page = 0;
